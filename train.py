...
from utils import CharNGramLM, beam_search_ctc_with_lm
...
lm_alpha = 0.5  # вес LM при декодировании
...
def run_validation(epoch, imgs_list, preds_list, truths_list, raws_list, lens_list):
    ...
    with torch.no_grad():
        for imgs, labs_cat, _, lab_lens, dec_inputs, targets in tqdm(
            ...
            attn_val_loss += F.cross_entropy(
                attn_out.view(-1, V), targets.view(-1), ignore_index=0
            ).item()

            preds, raws = beam_search_ctc_with_lm(
                ctc_out,
                alphabet,
                lm_model,
                alpha=lm_alpha,
    ...
    print(
        f"[Val {epoch}] loss={avg_val_loss:.4f} Acc={val_acc:.4f} CER={val_cer:.4f} WER={val_wer:.4f}"
    )
    return avg_val_loss, val_acc, val_cer, val_wer
...
for epoch in range(1, epochs + 1):
    ...
    for imgs, labs_cat, inp_lens, lab_lens, dec_inputs, targets in tqdm(
        ...
        preds, raws = decode(ctc_out, alphabet, method=decode_type_train, beam_width=5)
        offset = 0
        for length in lab_lens.tolist():
            seq = labs_cat[offset : offset + length].tolist()
            offset += length
    ...
    if epoch % val_interval == 0:
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(
                model.state_dict(), os.path.join(checkpoint_dir, "best_by_loss.pth")
            )
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            torch.save(
                model.state_dict(), os.path.join(checkpoint_dir, "best_by_acc.pth")
            )
        ...
        worst_imgs, worst_preds, worst_truths, worst_raws, worst_lens = (
            get_worst_examples(
                imgs_all_val,
                preds_all_val,
                truths_all_val,
        ...
        # only print training stats when skipping validation
        print(
            f"Epoch {epoch}/{epochs} | "
            f"TrainLoss={avg_train_loss:.4f} Acc={train_acc:.4f} "
            f"CER={train_cer:.4f} WER={train_wer:.4f} | No validation"
        )
writer.close()
