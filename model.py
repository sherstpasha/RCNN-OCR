...
class Attention(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes,
                 sos_id: int, eos_id: int, pad_id: int, blank_id: int | None = None):
        super().__init__()
    ...
    def forward(self, batch_H, text=None, is_train=True, batch_max_length=25):
        ...
        assert text is not None, "For training, `text` with <SOS> at text[:,0] is required"
        device = batch_H.device
        B = batch_H.size(0)
        steps = batch_max_length + 1
...
class RCNN(nn.Module):
    def __init__(self, num_classes, hidden_size=256,
                 sos_id: int = 1, eos_id: int = 2, pad_id: int = 0, blank_id: int | None = 3,
    ...
    def forward(self, x, text=None, is_train=True, batch_max_length=25):
        enc = self.encode(x)
        return self.attn(enc, text=text, is_train=is_train, batch_max_length=batch_max_length)